...
public class AccessControlService : IAccessControlService
{
    ...
    public async Task<string> RefreshTokenExists(string refreshToken)
    {
        var user = await _userManager.Users.FirstOrDefaultAsync(x =>
            x.RefreshToken == refreshToken && x.RefreshTokenExpireTime > DateTime.UtcNow);
        return user?.Email;
    }
    ...
    public async Task<bool> SetUserRefreshToken(string username, string refreshToken, TimeSpan expirationTime)
    {
        try
        {
            ...
            user.RefreshTokenExpireTime = DateTime.UtcNow.Add(expirationTime);
    ...
    public string GenerateJWTToken(Claim[] claims)
    {
        ...
        var tokenDescriptor = new SecurityTokenDescriptor
        {
            ...
            Expires = DateTime.UtcNow.AddMinutes(_jwtSettings.JWTExpirationTime),
