...
public class CommonConfigBatchQueryQueueService : BackgroundService
{
        ...
        foreach (var batchQueueOperationGroup in arrayPoolCollection
            ...
            try
            {
                ...
                _logger.LogInformation(
                    $"QueryParameters:{argument.QueryParameters},Type:{argument.Type},{batchQueueOperationGroup.Count()} requests");
                ...
                var task =
                    ((Func<PaginationQueryParameters, Task<ListQueryResult<object>>>)func).Invoke(
                        argument.QueryParameters);
                result = await task;
                foreach (var operation in batchQueueOperationGroup) operation.SetResult(result);
            }
            catch (Exception ex)
            {
    ...
    private Expression<Func<PaginationQueryParameters, Task<ListQueryResult<object?>>>>
        CreateQueryByParameterLambdaExpression(Type type)
    {
        ...
        return Expression.Lambda<Func<PaginationQueryParameters, Task<ListQueryResult<object?>>>>(callExpr,
            queryParameters);
    }

    private Expression<Func<IEnumerable<string>, Task<ListQueryResult<object?>>>>
    ...
    private async Task<ListQueryResult<object>> QueryByQueryParametersAsync<T>(
        ...
        where T : ModelBase, IAggregateRoot
    {
        ...
        var result =
            await repo.ListAsync(new CommonConfigSpecification<T>(DataFilterCollection<string>.Includes(idList)));
